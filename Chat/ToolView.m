...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        UIColor *color = [UIColor colorWithRed:246.0/255 green:246.0/255 blue:247.0/255 alpha:1];
...
-(void) addSubview
{
    ...
    self.changeKeyBoardButton = [[UIButton alloc] initWithFrame:CGRectZero];  //笑脸表情图片
    ...
    self.moreButton = [[UIButton alloc] initWithFrame:CGRectZero];//+号图片
    ...
    [self.moreView setMoreBlock:^(NSInteger index) {
        ...
        copy_self.extendBlock((int)index);   //不用强制转化为int
    }];
